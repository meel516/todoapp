  ...
  const {userDetails,Logout} =useAuth()
    ...
    <nav className='p-3 bg-slate-200 w-full sticky flex justify-between items-center'>
        ...
        <div className='rounded-md flex py-2 px-3 items-center gap-2'>
            ...
            <Pops className={"top-[40px] right-[40px] bg-slate-200"} wrapper={  <div className='rounded-full text-2xl w-[40px] aspect-square bg-slate-300 select-none inline-flex justify-center cursor-pointer items-center'>{userDetails?.username?.charAt(0)}</div>}>
